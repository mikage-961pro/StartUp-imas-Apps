...
class ViewController2: NSViewController {
    ...
    @IBOutlet var miku_clock: WKWebView!

    
    var cookies = Dictionary<String,String>()
    
    var miku_clock_webView: WKWebView!

    
    override func viewDidLoad() {
        super.viewDidLoad()
        let url = "https://million.765pro-theater.com/"
        ...
        miku_clock_webView.load(request)
        
    
        let cookies = HTTPCookieStorage.shared.cookies ?? []
        for (cookie) in cookies {
            miku_clock_webView.configuration.websiteDataStore.httpCookieStore.setCookie(cookie)
            print("Set Cookie OK")
        }
